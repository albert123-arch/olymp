...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach (admin_languages() as $lang) {
        $c = $lang['code'];
        if (column_exists('chapter_texts', 'description_html')) {
            ...
            execute_query('INSERT INTO chapter_texts (chapter_id, lang, title, summary_html, theory_html, examples_html, worksheet_html, teacher_notes_html) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE title=VALUES(title), summary_html=VALUES(summary_html), theory_html=VALUES(theory_html), examples_html=VALUES(examples_html), worksheet_html=VALUES(worksheet_html), teacher_notes_html=VALUES(teacher_notes_html)', [$id, $c, $_POST["title_$c"] ?? '', $_POST["description_$c"] ?? '', $_POST["theory_$c"] ?? '', $_POST["examples_$c"] ?? '', $_POST["worksheet_$c"] ?? '', $_POST["teacher_$c"] ?? '']);
        }
    }
...
<?= admin_textarea("description_$c", $txt['description_html'] ?? $txt['summary_html'] ?? '', 3) ?><?= admin_textarea("theory_$c", $txt['theory_html'] ?? '', 6) ?><?= admin_textarea("examples_$c", $txt['examples_html'] ?? '', 5) ?><?= admin_textarea("worksheet_$c", $txt['worksheet_html'] ?? '', 4) ?><?= admin_textarea("teacher_$c", $txt['teacher_notes_html'] ?? '', 4) ?>

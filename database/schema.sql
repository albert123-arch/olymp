...
INSERT INTO course_texts (course_id, lang, title, summary_html, overview_html, teacher_guide_html)
SELECT id, 'ru',
CASE slug WHEN 'number-theory' THEN '?????? ?????' WHEN 'algebra' THEN '???????' WHEN 'geometry' THEN '?????????' WHEN 'combinatorics' THEN '?????????????' WHEN 'inequalities' THEN '???????????' WHEN 'functional-equations' THEN '?????????????? ?????????' ELSE '????????? ??????' END,
CASE slug WHEN 'number-theory' THEN '?????????, ??????? ?????, ?????????, ?????????? ????????? ? ???????????? ??????????? ??????.' WHEN 'algebra' THEN '?????????, ??????????, ??????????????????, ????????? ? ??????????? ???????.' WHEN 'geometry' THEN '????????????, ??????????, ???????, ?????????????? ? ?????? ??????????????.' WHEN 'combinatorics' THEN '???????, ??????? ???????, ??????????, ?????????, ????? ? ????.' WHEN 'inequalities' THEN 'AM-GM, ????, ???????????? ? ??????????? ???????????.' WHEN 'functional-equations' THEN '???????????, ?????????????, ?????????????? ? ????????? ???????.' ELSE '????????? ???????? ????? ? ??????? ?????????.' END,
CASE slug WHEN 'number-theory' THEN '<p>??????? ? ????????? ? ?????????? ?? ??????? ?????????, ????? ?????????? ? ?????????? ? ???????????? ????????.</p>' ELSE '<p>???? ???? ???????????? ??? ?????????? ?????.</p>' END,
CASE slug WHEN 'number-theory' THEN '<p>??????? ????????? ? ?????????????, ????? ??????? ???????? ????????????? ??????? ????????????? ????? ??????????? ??????.</p>' ELSE '<p>???????????? ??????? ???????? ?????? ? ?????? ?????????????? ??????.</p>' END
FROM courses;
...
INSERT INTO chapter_texts (chapter_id, lang, title, summary_html, theory_html, examples_html, worksheet_html, teacher_notes_html)
SELECT id, 'ru',
CASE slug WHEN 'divisibility-prime-factorisation' THEN '????????? ? ?????????? ?? ??????? ?????????' WHEN 'gcd-lcm-euclidean-algorithm' THEN '???, ??? ? ???????? ???????' WHEN 'modular-arithmetic' THEN '????????? ??????????' WHEN 'congruences-remainders' THEN '????????? ? ???????' WHEN 'diophantine-equations' THEN '?????????? ?????????' WHEN 'infinite-descent' THEN '??????????? ?????' ELSE '????? ? ?????' END,
CASE slug WHEN 'divisibility-prime-factorisation' THEN '???????? ???????????, ?????????? ?? ??????? ?????????, ??????? ????????? ? ???????? ??????????????.' ELSE '?????.' END,
CASE slug WHEN 'divisibility-prime-factorisation' THEN '<h2>?????????</h2><p>??? ????? \\(a\\) ? \\(b\\), ??? \\(a\\ne0\\), ?????? \\(a\\mid b\\) ????????, ??? \\(b=ak\\) ??? ?????????? ?????? \\(k\\).</p><h2>?????????? ?? ??????? ?????????</h2><p>?????? ????? \\(n>1\\) ???????????? ??????? ?????????????? ??? ???????????? ???????? ??????? ?????.</p><h2>??????? ?????????</h2><p>???? \\(n=p_1^{a_1}\\cdots p_m^{a_m}\\), ?? \\(\\tau(n)=(a_1+1)\\cdots(a_m+1)\\).</p>' ELSE '<p>???????? ????? ???????? ?????.</p>' END,
CASE slug WHEN 'divisibility-prime-factorisation' THEN '<ol><li>???? \\(6\\mid n\\), ???????? \\(3\\mid n\\).</li><li>??????? ?????????? \\(840\\) ?? ??????? ?????????.</li><li>?????????? ????????????? ???????? \\(840\\).</li></ol>' ELSE '<p>??????? ????? ????????? ?????.</p>' END,
CASE slug WHEN 'divisibility-prime-factorisation' THEN '<p>? ???? MVP ??????? ???????? ?????? ??????? ??????. ???????? ??????? ????? ???????? ?????.</p>' ELSE '<p>??????? ???? ????? ???????? ?????.</p>' END,
CASE slug WHEN 'divisibility-prime-factorisation' THEN '<p>????????? ????????????? ? ??????????? \\(b=ak\\). ??????????? ?????? ??????????????, ?????????? ?? ????????? ? ?????????????? ?? ??????????.</p>' ELSE '<p>???????????? ??????? ????? ????????? ?????.</p>' END
FROM chapters;
